...
# Gets the fifty most recent posts, starting at the requested number, within five miles of the requested location.
#
# Parameters:
# - latitude: The latitude of the desired location.
# - longitude: The longitude of the desired location.
# - startIndex: The index of the first post to return, e.g. posts[50...99]
@https_fn.on_call()
def getLatestPosts(req: https_fn.CallableRequest) -> any:
    # Query Firestore for the list of post documents sorted by most to least recent!
    orderedPosts = firestore.client().collection("posts").order_by("timePosted", direction = firestore.Query.DESCENDING).stream()

    # Loop through each post and decide if it is close enough to the location!
    acceptedPosts = []; rangedPostIndex = -1
    for eachPost in orderedPosts:
        postLocation = (float(eachPost.to_dict()["latitude"]), float(eachPost.to_dict()["longitude"]))
        desiredLocation = (float(req.data["latitude"]), float(req.data["longitude"]))

        if haversine(postLocation, desiredLocation, unit = Unit.MILES) <= 5:
            postsWithinRange += 1
            
            if postsWithinRange >= int(req.data["startIndex"]):
                acceptedPosts.append(json.dumps(eachPost.to_dict(), default = str))
...
def testFunction(req: https_fn.CallableRequest) -> any:
    ...
    rawPosts = []
    girlWhat = ["one", "two", "three"]
    numPosts = 0
    for eachPost in orderedPosts:
        acceptedPosts.append(eachPost.id)
        # rawPosts.append(f"{eachPost.to_dict()}")
        rawPosts.append(json.dumps(eachPost.to_dict(), default = str))
        numPosts += 1
